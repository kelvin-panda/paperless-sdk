...
//----------会议类型----------
#define TYPEMEET_LITTLEMASK    0x1F //会议类型低位掩码 保存会议类型 - 最大31种会议
#define TYPEMEET_BIGMASK       0xE0 //会议类型高位掩码 保存会议属性

#define type_meeting_autoclose 0x10 //自动结束
#define type_meeting_secret    0x20 //保密
#define type_meeting_autostart 0x40 //自动开始

#define type_meeting_normal    0x00 //一般会议
#define type_meeting_executive 0x01 //机密会议-兼容以前的保密会议
#define type_meeting_Party     0x02 //党委会
#define type_meeting_Standing  0x03 //常委会
#define type_meeting_Director  0x04 //董事会
#define type_meeting_Special   0x05 //专委会
#define type_meeting_Finance   0x06 //财审会
#define type_meeting_gov       0x07 //政务会
#define type_meeting_law       0x08 //法务会
#define type_meeting_company   0x09 //企业会
#define type_meeting_school    0x0a //学校会
#define type_meeting_org       0x0b //机构会
#define type_meeting_free      0x0c //自由会

#define GET_MEET_HTYPE(T)  (T & TYPEMEET_BIGMASK) //获取会议标志
#define GET_MEET_TYPE(T)  (T & TYPEMEET_LITTLEMASK) //获取会议类型
#define IS_SECRET_MEET(T)  (((T & TYPEMEET_LITTLEMASK) == type_meeting_executive) || (T & type_meeting_secret)) //判断会议是否有保密属性
...
#define signin_late					0x14	//迟到
#define signin_complex				0x15	//复合签到--将类型放到数据里，数据里增加一个结构头PD_ComplexSignInHdrInfo
...
#define AVOTEBASE_FLAG_LAST		0x0000080 //表示投票末位表决
...
#define COMPLEX_PUBLIC_TYP_MEET    4//typ=4指会议,id1是会议id
#define COMPLEX_PUBLIC_TYP_FILE    5//typ=5指平台文件，文件保存公共的数据,id1是文件id
...
#define CMX_PUBFILE_DATAID_BASE 1 //文件的基本信息
/*
{
}
*/
...
{
"pagenum":3,//页码
"dirid":3,//针对批注文件，保存原始的目录ID
"fileid":"0x6b000001",//针对批注文件，保存原始的目录ID
}
*/
